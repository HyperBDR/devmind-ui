...
test.describe('LLM pages', () => {
  test.beforeEach(async ({ page }) => {
    ...
    if (url.includes('/login')) {
      await page.fill(
        'input[type="text"], input[name="username"], input[placeholder*="mail" i]',
      ...
      await page.click(
        'button[type="submit"], input[type="submit"], button:has-text("登录"), button:has-text("Login")'
      )
      await page.waitForLoadState('networkidle')
    }
  })
  ...
  test('LLM Stats page shows summary and filters', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle')
    await expect(
      page.locator('text=Token').or(page.locator('text=统计')).first()
