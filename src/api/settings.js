...
export const settingsApi = {
  ...
  async getWebhookConfig() {
    ...
    const webhookConfigSetting = settingsData.find(
      (s) => s.key === 'webhook_config'
    )

    return {
      enable: webhookConfigSetting?.is_active ?? false,
      url: webhookConfigSetting?.value?.url || '',
      provider: webhookConfigSetting?.value?.provider || 'wechat',
      language: webhookConfigSetting?.value?.language || 'zh-hans',
      timeout: webhookConfigSetting?.value?.timeout || 10,
      retries: webhookConfigSetting?.value?.retries || 3,
      headers: webhookConfigSetting?.value?.headers || {},
      events: webhookConfigSetting?.value?.events || []
    }
  },
  ...
  async updateWebhookConfig(config) {
    ...
    if (webhookConfigSetting) {
      return await this.updateSetting(webhookConfigSetting.id, {
        value: webhookValue,
        is_active: config.enable ?? false
      })
    } else {
      return await this.createSetting({
        key: 'webhook_config',
  ...
  async validateWebhookConfig(config) {
    try {
      const response = await apiClient.post(
        '/articlehub/settings/validate-webhook',
        config
      )
      return response.data
    } catch (error) {
      // Return error response data if available, otherwise rethrow
      if (error.response?.data) {
        // If the error response has the standard format, return it
        // Otherwise wrap it in the expected format
        const errorData = error.response.data
        if (errorData.data || errorData.message || errorData.code) {
          return errorData
        }
