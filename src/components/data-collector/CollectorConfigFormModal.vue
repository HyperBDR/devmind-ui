  ...
  >
    <div class="space-y-6">
      ...
      <form v-if="currentStep === 1" @submit.prevent="canGoNextFromStep1 && (currentStep = 2, loadProjectsIfNeeded())" class="space-y-4">
        ...
        <template v-else-if="formData.platform === 'ai_pricehub'">
          <div class="space-y-3 border-b border-gray-200 pb-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-start">
              ...
              <div class="md:col-span-2">
                <BaseInput v-model="formData.ai_pricehub_platform_slug" type="text" class="w-full" />
              </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-start">
              <div class="md:col-span-1">
              ...
              <div class="md:col-span-2">
                <BaseInput v-model="formData.ai_pricehub_vendor_name" type="text" class="w-full" />
              </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-start">
              <div class="md:col-span-1">
              ...
              <div class="md:col-span-2">
                <BaseInput v-model="formData.ai_pricehub_endpoint_url" type="url" class="w-full" />
              </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-start">
              <div class="md:col-span-1">
              ...
              <div class="md:col-span-2">
                <BaseInput v-model="formData.ai_pricehub_region" type="text" class="w-full" />
              </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-start">
              <div class="md:col-span-1">
              ...
              <div class="md:col-span-2">
                <BaseInput v-model="formData.ai_pricehub_currency" type="text" class="w-full" />
              </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-start">
              <div class="md:col-span-1">
              ...
              <div class="md:col-span-2">
                <BaseInput v-model.number="formData.ai_pricehub_points_per_currency_unit" type="number" min="0" step="0.01" class="w-full" />
              </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-start">
              <div class="md:col-span-1">
              ...
              <div class="md:col-span-2">
                ...
                >
                  <option value="">
                    {{ aiPriceHubLlmOptionsLoading ? t('dataCollector.aiPriceHub.parserLlmLoading') : t('dataCollector.aiPriceHub.parserLlmPlaceholder') }}
                  </option>
                  <option
                ...
                <p v-if="aiPriceHubLlmLoadError" class="mt-1 text-xs text-red-600">
                  {{ aiPriceHubLlmLoadError }}
                </p>
                <p
                  v-else-if="!aiPriceHubLlmOptionsLoading && aiPriceHubLlmSelectOptions.length === 0"
        ...
        <div v-if="formData.platform === 'jira' || formData.platform === 'feishu' || formData.platform === 'license' || formData.platform === 'ai_pricehub'" class="space-y-1 pt-2">
          ...
          <div v-else class="max-h-64 overflow-y-auto rounded border border-gray-200 divide-y divide-gray-200">
            ...
            >
              <input
                ...
                class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
              />
              <span class="font-medium text-gray-900">{{ proj.name || proj.key }}</span>
...
const aiPriceHubLlmSelectOptions = computed(() => {
  const base = Array.isArray(aiPriceHubLlmOptions.value) ? [...aiPriceHubLlmOptions.value] : []
...
const step1AuthFieldsFilled = computed(() => {
  ...
  if (formData.platform === 'ai_pricehub') {
    return !!(formData.ai_pricehub_platform_slug || '').trim()
      ...
      primary_sources: [
        {
          ...
          parser_llm_config_uuid: (formData.ai_pricehub_parser_llm_config_uuid || '').trim(),
          currency: (formData.ai_pricehub_currency || '').trim() || 'CNY',
          points_per_currency_unit: Number(formData.ai_pricehub_points_per_currency_unit) || 10.0,
          is_enabled: true,
          notes: '',
        },
      ],
...
function applyConfig(c) {
  ...
  formData.selected_project_keys = Array.isArray(v.project_keys)
    ...
    : (formData.platform === 'feishu'
        ...
        : (formData.platform === 'ai_pricehub' ? ['sync'] : []))
  const primarySource = Array.isArray(v.primary_sources) && v.primary_sources.length > 0
    ? v.primary_sources[0]
    : null
  ...
  formData.ai_pricehub_points_per_currency_unit = Number(primarySource?.points_per_currency_unit) || 10.0
  formData.ai_pricehub_parser_llm_config_uuid = primarySource?.parser_llm_config_uuid || ''
...
watch(
  () => ({
    ...
    ai_pricehub_points_per_currency_unit: formData.ai_pricehub_points_per_currency_unit,
    ai_pricehub_parser_llm_config_uuid: formData.ai_pricehub_parser_llm_config_uuid
  }),
  () => {
    step1AuthVerified.value = false
    authError.value = ''
...
async function loadAiPriceHubLlmOptions() {
  ...
  try {
    ...
    const rows = Array.isArray(payload?.options) ? payload.options : (Array.isArray(payload) ? payload : [])
    ...
    aiPriceHubLlmLoadError.value = extractErrorMessage(e, t('dataCollector.aiPriceHub.parserLlmLoadError'))
...
async function loadProjectsIfNeeded() {
  ...
  try {
    if (formData.platform === 'ai_pricehub') {
      projects.value = [{ key: 'sync', id: 'sync', name: t('dataCollector.aiPriceHub.syncCollection') }]
      if (!Array.isArray(formData.selected_project_keys) || formData.selected_project_keys.length === 0) {
