<template>
  <BaseModal :show="show" :title="modalTitle" @close="handleClose">
    ...
    <div
      class="sticky top-0 z-50 bg-white border-b border-gray-200 pb-3 mb-4 -mx-4 -mt-4 px-4 pt-3 shadow-sm"
    >
      <div
        class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-3 space-y-2"
      >
        <!-- Task Name (最多两行显示) -->
        <div>
          ...
          <div class="text-sm text-blue-900 font-mono break-words line-clamp-2">
            {{
              autoGeneratedName ||
              formData.name ||
              t('scheduledTasks.form.nameAutoGenerated')
            }}
          </div>
        </div>
        ...
        <div class="flex items-center gap-4 text-sm">
          <div v-if="schedulePreview" class="flex-1">
            <div class="text-xs text-blue-700 font-medium mb-1">
              {{ t('scheduledTasks.form.schedulePreview') }}
            </div>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <div>
        ...
        >
          <option value="">
            {{ t('scheduledTasks.form.selectTaskType') }}
          </option>
          <option
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select v-model="formData.kwargs.lang_code" class="input w-full">
              <option value="any">
                {{ t('scheduledTasks.form.langCodeAny') }}
              </option>
              <option value="zh">
                {{ t('scheduledTasks.form.langCodeZh') }}
              </option>
              <option value="en">
                {{ t('scheduledTasks.form.langCodeEn') }}
              </option>
            </select>
          </div>
          ...
          <div>
            ...
            <select v-model="formData.kwargs.since" class="input w-full">
              <option value="today">
                {{ t('scheduledTasks.form.trendingPeriodToday') }}
              </option>
              <option value="weekly">
                {{ t('scheduledTasks.form.trendingPeriodWeekly') }}
              </option>
              <option value="monthly">
                {{ t('scheduledTasks.form.trendingPeriodMonthly') }}
              </option>
            </select>
          </div>
      ...
      <div v-if="!isGithubTrendingTask" class="space-y-4">
        ...
        <div>
          ...
          <div
            class="inline-flex rounded-md bg-gray-100 p-0.5 shadow-sm border border-gray-200"
            role="group"
          >
            <button
              type="button"
        ...
        <div v-if="searchType === 'keywords'">
            ...
            >
              <div class="flex-1 relative">
                <input
                  ...
                  type="text"
                  :placeholder="
                    t('scheduledTasks.form.keywordPlaceholder', {
                      num: index + 1
                    })
                  "
          ...
          <p class="text-sm text-gray-500 mt-1">
            {{ t('scheduledTasks.form.keywordsHelp') }} ({{
              t('scheduledTasks.form.maxKeywords', { count: 4 })
            }})
          </p>
      ...
      <div class="bg-gray-50 rounded-lg p-4 space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <p
              v-else-if="
                formData.task_type ===
                'articlehub.tasks.monitor.alert_unpublished_articles'
              "
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="flex-1">
              ...
              <p class="text-xs text-blue-700 mt-1">
                {{
                  t('scheduledTasks.form.timezoneDescription', {
        ...
        <div
          v-if="
            isGithubTrendingTask ||
            formData.task_type === 'articlehub.collect_articles'
          "
      ...
      >
        ...
        <div>
          ...
          <select v-model="formData.kwargs.source_region" class="input w-full">
    ...
    availableTaskTypes.value = [
      {
        ...
        description:
          'Collect articles from Google News based on keywords or topics'
      }
    ]
...
const autoGeneratedName = computed(() => {
  ...
  if (selectedTaskType.value?.label) {
    ...
    const taskTypeName =
      formData.value.task_type.split('.').pop()?.replace(/_/g, '') || 'Task'
    parts.push(taskTypeName)
  }
  ...
  if (formData.value.task_type === 'github_trending.collect_github_trendings') {
    ...
    if (keywords.length > 0) {
      ...
      formData.value.kwargs.keywords &&
      formData.value.kwargs.keywords.length > 0
    ) {
      const fallbackKeywords = formData.value.kwargs.keywords
        .slice(0, 3)
        .join('_')
    ...
    if (
      formData.value.kwargs.topics &&
      formData.value.kwargs.topics.length > 0
    ) {
    ...
    if (
      formData.value.kwargs.social_account &&
      formData.value.kwargs.social_account.trim()
    ) {
...
const validateCrontab = () => {
  ...
  const isAlertTask =
    taskType === 'articlehub.tasks.monitor.alert_unpublished_articles'
  ...
  if (
    isGithubTrendingTask ||
    formData.value.task_type === 'articlehub.collect_articles'
  ) {
    ...
    if (monthOfYear !== '*') {
      ...
      for (const m of monthValues) {
        ...
        if (isNaN(num) || num < 1 || num > 12) {
          errors.value.month_of_year = t(
            'scheduledTasks.form.monthOfYearInvalid'
          )
          return false
        }
      }
...
const validateKeyword = (index) => {
  ...
  if (keyword && keyword.length > maxLength) {
    errors.value[`keyword_${index}`] = t('scheduledTasks.form.keywordTooLong', {
      max: maxLength
    })
    return false
  }
  ...
  const duplicates = keywordInputs.value.filter(
    (k, i) =>
      i !== index &&
      k.trim().toLowerCase() === trimmedKeyword &&
      trimmedKeyword !== ''
  )
...
const updateKeywords = () => {
  ...
  const keywords = keywordInputs.value.map((k) => k.trim()).filter((k) => k)
  ...
  keywordInputs.value.forEach((keyword, index) => {
    ...
    if (trimmed && trimmed.length > maxLength) {
      errors.value[`keyword_${index}`] = t(
        'scheduledTasks.form.keywordTooLong',
        { max: maxLength }
      )
      hasError = true
    }
  ...
  keywordInputs.value.forEach((keyword, index) => {
    ...
    if (trimmed) {
      const duplicates = keywordInputs.value.filter(
        (k, i) => i !== index && k.trim().toLowerCase() === trimmed
      )
      if (duplicates.length > 0) {
        errors.value[`keyword_${index}`] = t(
          'scheduledTasks.form.keywordDuplicate'
        )
        hasError = true
      }
    }
...
const loadTask = async () => {
  if (props.task) {
    ...
    if (
      props.task.uuid &&
      (!props.task.kwargs || Object.keys(props.task.kwargs || {}).length === 0)
    ) {
      try {
        const response = await scheduledTasksApi.getScheduledTask(
          props.task.uuid
        )
    ...
    formData.value = {
      ...
      start_time: taskData.start_time
        ? formatDateTimeForInput(taskData.start_time)
        : getCurrentDateTime(),
      kwargs: {
        ...
        transform_formats: kwargs.transform_formats || [
          'markdown_json',
          'toutiao'
        ],
    ...
    if (
      ...
      selectedTopic.value =
        topics.length > 0 && availableTopics.includes(topics[0])
          ? topics[0]
          : ''
...
watch(
  ...
  (newName) => {
    if (!props.task && newName) {
      ...
      if (
        !formData.value.name ||
        formData.value.name === previousAutoName.value
      ) {
...
watch(
  ...
  () => {
    if (
      formData.value.task_type === 'github_trending.collect_github_trendings'
    ) {
...
const validate = () => {
  ...
  if (
    !formData.value.kwargs.transform_formats ||
    formData.value.kwargs.transform_formats.length === 0
  ) {
    errors.value.transform_formats = t(
      'scheduledTasks.form.transformFormatsRequired'
    )
  }
...
const handleSubmit = () => {
  ...
  const dayOfMonth =
    formData.value.schedule_value?.day_of_month ||
    crontabInput.value.day_of_month?.trim() ||
    '*'
  const monthOfYear =
    formData.value.schedule_value?.month_of_year ||
    crontabInput.value.month_of_year?.trim() ||
    '*'
  const dayOfWeek =
    formData.value.schedule_value?.day_of_week ||
  ...
  if (
    ...
    !Array.isArray(submitData.kwargs.keywords)
  ) {
    submitData.kwargs.keywords = []
  }
  ...
  if (
    !submitData.kwargs.transform_formats ||
    !Array.isArray(submitData.kwargs.transform_formats)
  ) {
