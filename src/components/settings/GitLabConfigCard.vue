    ...
    <div class="px-4 py-3 border-b border-gray-200 bg-gray-50">
      <div class="flex items-center gap-2 text-gray-800">
        ...
        <h3 class="text-sm font-semibold">
          {{ t('settings.gitlab.configTitle') }}
        </h3>
      </div>
    </div>
    <div class="p-4">
      <p class="text-xs text-gray-600 mb-3">
        {{ t('settings.gitlab.configDesc') }}
      </p>
        ...
        <div class="flex items-center justify-between">
          <div>
            <label class="text-sm font-medium text-gray-700">
              {{ t('settings.gitlab.enable') }}
            </label>
            <p class="text-xs text-gray-500 mt-0.5">
              {{ t('settings.gitlab.enableDesc') }}
            </p>
          </div>
        ...
        <template v-if="localConfig.enable">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-start">
            <div class="md:col-span-1">
              ...
              >
                {{ t('settings.gitlab.serverUrl') }}
              </label>
              <p class="text-xs text-gray-500 mb-2 md:mb-0">
                {{ t('settings.gitlab.serverUrlDesc') }}
              </p>
            </div>
            <div class="md:col-span-2">
              <input
                ...
                type="url"
                :placeholder="t('settings.gitlab.serverUrlPlaceholder')"
...
const handleValidate = async () => {
  ...
  try {
    const response = await settingsApi.validateGitLabConfig(localConfig)
    ...
    console.error('Failed to validate GitLab config:', err)
...
const saveConfig = async () => {
  ...
  try {
    await settingsApi.updateGitLabConfig(localConfig)
    success.value = t('settings.gitlab.saveSuccess')
    setTimeout(() => {
      success.value = ''
    }, 3000)
  } catch (err) {
