    ...
    <div class="w-full max-w-full p-6 space-y-6">
      ...
      >
        ...
        >
          ...
          <div class="w-full max-w-md">
            <HyperBDRDataSourceSelect layout="inline" variant="toolbar" />
          </div>
        </div>
      </section>
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
        <article
          ...
          @click="openDashboardLink('HyperBDRMonitorTenants')"
        >
        ...
        <article
          ...
          @click="openDashboardLink('HyperBDRMonitorHosts')"
        >
          <div class="flex items-start justify-between gap-3">
            <div>
              <p
        ...
        <article
          ...
          @click="openDashboardLink('HyperBDRMonitorLicenses')"
        >
          <div class="flex items-start justify-between gap-3">
            <div>
              <p
      ...
      >
        ...
        <div class="grid gap-4 px-6 py-5 md:grid-cols-3">
          <div
            ...
            @click="
              openDashboardLink('HyperBDRMonitorTasks', { status: 'failed' })
            "
          >
          ...
          <div
            ...
            @click="
              openDashboardLink('HyperBDRMonitorHosts', {
                health_scope: 'risky'
              })
            "
          >
...
import { hyperbdrMonitorApi } from '@/api/hyperbdrMonitor'
import AppLayout from '@/components/layout/AppLayout.vue'
import { useToast } from '@/composables/useToast'

import { toNumber, unwrap } from './shared'
import HyperBDRDataSourceSelect from './HyperBDRDataSourceSelect.vue'
import { useHyperBDRDataSourceScope } from './useHyperBDRDataSourceScope'

const { t } = useI18n()
const { showError } = useToast()
const router = useRouter()
const { selectedDataSource, loadDataSources, appendDataSourceScope } =
  useHyperBDRDataSourceScope()
...
async function loadDashboard() {
  const response = await hyperbdrMonitorApi.dashboard(queryParams())
