...
import HyperBDRPageHeader from './HyperBDRPageHeader.vue'
import {
  formatDateTime,
...
async function loadHosts() {
  loading.value = true
  try {
    const response = await hyperbdrMonitorApi.hosts.list(
      appendDataSourceScope({
...
async function exportHosts(scope) {
  ...
  try {
    await hyperbdrMonitorApi.hosts.export(buildExportParams(scope))
