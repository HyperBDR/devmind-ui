    ...
    <div class="w-full max-w-full p-6 space-y-6">
      ...
      <div class="bg-white border border-gray-200 rounded-xl shadow-sm">
        ...
        >
          ...
          <router-link
            :to="{ name: 'HyperBDRMonitorSettingsDataSources' }"
...
async function loadSource() {
  ...
  try {
    const response = await hyperbdrMonitorApi.dataSources.get(route.params.id)
...
async function submit() {
  ...
  try {
    ...
    if (isEdit.value) {
      await hyperbdrMonitorApi.dataSources.update(route.params.id, payload)
      showSuccess(t('hyperbdrMonitor.settingsPage.updateSuccess'))
    } else {
      await hyperbdrMonitorApi.dataSources.create(payload)
      showSuccess(t('hyperbdrMonitor.settingsPage.createSuccess'))
    }
    router.push({ name: 'HyperBDRMonitorSettingsDataSources' })
  } catch (error) {
    showError(
      error?.response?.data?.detail ||
