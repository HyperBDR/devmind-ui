    ...
    <div class="w-full max-w-full p-6 space-y-6">
      ...
      <div class="bg-white border border-gray-200 rounded-xl shadow-sm">
        ...
        >
          ...
          <router-link
            :to="{ name: 'HyperBDRMonitorSettingsDataSourceCreate' }"
...
async function loadDataSources() {
  loading.value = true
  try {
    const response = await hyperbdrMonitorApi.dataSources.list({ limit: 200 })
...
async function toggleStatus(source) {
  try {
    await hyperbdrMonitorApi.dataSources.patch(source.id, {
...
async function triggerCollect(id) {
  try {
    await hyperbdrMonitorApi.dataSources.collect(id)
...
async function removeSource(source) {
  ...
  try {
    await hyperbdrMonitorApi.dataSources.delete(source.id)
