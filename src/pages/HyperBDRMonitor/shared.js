...
export const HYPERBDR_HOST_STATUS_OPTIONS = [
  'sync_snapshot_done',
  'host_register_done',
  'sync_failed',
  'clean_failed',
...
const HYPERBDR_HOST_STATUS_TONES = {
...
const HYPERBDR_BOOT_STATUS_TONES = {
...
const HYPERBDR_HEALTH_STATUS_TONES = {
...
export function oneProHostStatusClass(status) {
  return (
    HYPERBDR_HOST_STATUS_TONES[String(status || '').toLowerCase()] ||
...
export function oneProBootStatusClass(status) {
  return (
    HYPERBDR_BOOT_STATUS_TONES[String(status || '').toLowerCase()] ||
    'bg-gray-100 text-gray-700 border-gray-200'
  )
}
...
export function oneProHealthStatusClass(status) {
  return (
    HYPERBDR_HEALTH_STATUS_TONES[String(status || '').toLowerCase()] ||
    'bg-gray-100 text-gray-700 border-gray-200'
  )
}
