      ...
      <section class="rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div class="border-b border-gray-200 px-6 py-5">
          <div class="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h2 class="text-lg font-semibold text-gray-900">{{ t('oneproMonitor.tenantsPage.filtersTitle') }}</h2>
              <p class="mt-1 text-sm text-gray-500">{{ t('oneproMonitor.tenantsPage.filtersSubtitle') }}</p>
            </div>
            <p class="text-sm text-gray-500">{{ t('oneproMonitor.tenantsPage.showingCount', { count: tenants.length }) }}</p>
          </div>
        </div>
        <div class="px-6 py-5">
          <div class="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_140px]">
            ...
            <div class="flex items-end">
              <BaseButton class="w-full" :loading="loading" @click="loadTenants">{{ t('oneproMonitor.actions.filter') }}</BaseButton>
            </div>
          </div>
        </div>
      </section>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{{ t('oneproMonitor.tenantsPage.hostCount') }}</th>
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{{ t('oneproMonitor.tenantsPage.licenseOverview') }}</th>
            ...
            <tbody class="divide-y divide-gray-100 bg-white">
              <tr v-for="tenant in tenants" :key="`${tenant.data_source_id}-${tenant.id}`">
                <td class="px-6 py-4 text-sm text-gray-900">
                  <div class="font-medium">{{ tenant.name }}</div>
                  <div class="mt-1 text-xs text-gray-500">{{ tenant.id }}</div>
                </td>
                <td class="px-6 py-4 text-sm">
                  <span class="inline-flex rounded-full border px-2.5 py-1 text-xs font-medium" :class="pillClass(tenant.status)">
                    {{ tenant.status || t('oneproMonitor.labels.unknown') }}
                  </span>
                </td>
                <td class="px-6 py-4 text-sm text-gray-600">{{ tenant.data_source_name || '-' }}</td>
                <td class="px-6 py-4 text-sm text-gray-600">{{ tenant.host_count || 0 }}</td>
                <td class="px-6 py-4 text-sm text-gray-600">
                  {{ t('oneproMonitor.tenantsPage.licenseSummary', { used: tenant.license_used || 0, remaining: tenant.license_remaining || 0 }) }}
                </td>
                <td class="px-6 py-4 text-sm text-gray-600">{{ formatDateTime(tenant.last_collected_at) }}</td>
              </tr>
              <tr v-if="tenants.length === 0">
                <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500">{{ t('oneproMonitor.tenantsPage.empty') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
async function loadTenants() {
  loading.value = true
  try {
    const response = await oneproMonitorApi.tenants.list(appendDataSourceScope({
      limit: 200,
    ...
    showError(error?.response?.data?.detail || t('oneproMonitor.tenantsPage.loadError'))
  } finally {
    loading.value = false
  }
}
